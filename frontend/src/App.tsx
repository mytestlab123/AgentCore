  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'sidebar-open' : ''}`}>
        ...
        <div className="workspace-label">{isIssue9Mode ? 'Issue #9 native key POC' : 'Issue #4 MVP'}</div>
        ...
        <div className="sidebar-footer">
          ...
          <a href={`https://github.com/mytestlab123/AgentCore/issues/${isIssue9Mode ? '9' : '4'}`} target="_blank" rel="noreferrer">Issue #{isIssue9Mode ? '9' : '4'} <span>-&gt;</span></a>
        </div>
      </aside>
      ...
      <div className="main-shell">
        ...
        <footer className="page-footer"><span>{isIssue9Mode ? 'Issue #9 native Bedrock API key POC' : 'Issue #4 internal AI platform POC'}</span><span>{isIssue9Mode ? 'Real AWS proof / retained demo setup' : isLiveMode ? 'Live Bedrock responses' : 'Clearly marked local simulation'}</span></footer>
      </div>
    </div>
  );
}
