...
import { AwsPlaygroundResult, compareModels, CompareResult, getIssue9Proof, getNovaKeys, Issue9ProofStatus, NovaKeysStatus, revealIssue9Key, revealNovaKey, runAwsPlayground, runPlatformText, startIssue9Proof } from './issue9-api';
...
export function Issue9PlaygroundPage() {
  ...
  const [model, setModel] = useState<'nova2' | 'nova_pro' | 'platform_haiku'>('nova2');
  ...
  const run = async () => {
    ...
    try { setResult(model === 'platform_haiku' ? await runPlatformText(prompt) : await runAwsPlayground(model, tool, prompt)); }
  ...
  return (
    <>
      ...
      {mode === 'single' ? <section className="playground-grid">
        <article className="panel prompt-panel"><label>Model</label><select value={model} onChange={(event) => { const next = event.target.value as typeof model; setModel(next); if (next === 'platform_haiku') setPrompt('Explain why a public S3 bucket is a security risk and give three remediation steps.'); }}><option value="nova2">Nova 2 Lite</option><option value="nova_pro">Nova Pro</option><option value="platform_haiku">PlatformAI Claude Haiku 4.5</option></select>{model === 'platform_haiku' ? <div className="notice compact-notice"><span>i</span><div><strong>Text-only provider</strong><p>AWS tools are disabled. No EC2, Inspector, or SSM data is sent to PlatformAI.</p></div></div> : <><label>AWS tool</label><select value={tool} onChange={(event) => { const next = event.target.value as typeof tool; setTool(next); setPrompt(toolPrompts[next]); }}><option value="ec2">EC2 inventory</option><option value="inspector">Inspector findings</option><option value="ssm">SSM secret access - deny proof</option></select></>}<label>Question</label><textarea rows={7} value={prompt} disabled={model !== 'platform_haiku' && tool === 'ssm'} onChange={(event) => setPrompt(event.target.value)} /><div className="prompt-footer"><small>{model === 'platform_haiku' ? 'GovTech PlatformAI External / text only.' : tool === 'ssm' ? 'The model is not called and no secret metadata is returned.' : 'Fixed allowlist; no arbitrary AWS commands.'}</small><button className="button button-primary" disabled={running} onClick={() => void run()}>{running ? 'Running...' : model !== 'platform_haiku' && tool === 'ssm' ? 'Prove AWS deny' : 'Run live proof'}</button></div></article>
        <article className="panel response-panel"><div className="response-heading"><div><p className="eyebrow">Live result</p><h2>{result ? (result.tool === 'ssm' ? 'AWS IAM policy boundary' : result.tool === 'text' ? 'PlatformAI Claude summary' : 'Nova summary') : 'Ready'}</h2></div>{result && <Badge tone={result.decision === 'ALLOW' ? 'green' : 'gray'}>{result.decision}</Badge>}</div>{result ? <><div className={result.decision === 'DENY' ? 'error-box denied-response' : 'response-copy'}>{result.decision === 'DENY' ? result.answer : <ReactMarkdown remarkPlugins={[remarkGfm]}>{result.answer}</ReactMarkdown>}</div><div className="metadata-grid"><div><span>Model</span><strong>{result.model}</strong></div><div><span>{result.tool === 'text' ? 'Input boundary' : 'Sanitized records'}</span><strong>{result.tool === 'text' ? 'Text only' : result.records.length}</strong></div><div><span>Tokens</span><strong>{result.inputTokens} in / {result.outputTokens} out</strong></div><div><span>Completion</span><strong>{result.stopReason === 'max_tokens' ? 'Truncated' : 'Complete'}</strong></div></div></> : <div className="empty-response"><span>&gt;_</span><strong>Select a model and AWS tool</strong><p>EC2 and Inspector return sanitized facts. SSM must return an AWS IAM deny with zero secret data.</p></div>}</article>
