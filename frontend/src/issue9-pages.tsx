...
export function Issue9PlaygroundPage() {
  ...
  const [model, setModel] = useState<'nova2' | 'platform_gemini' | 'platform_haiku'>('nova2');
  ...
  const run = async () => {
    ...
    try {
      ...
      else if (model === 'platform_gemini') setResult(await runPlatformTool('gemini-2.5-flash', tool, prompt));
